...
locals {
  ...
  template_variables = {
    affected_schemas   = local.schemas
  ...
  # Set all schemas as default value in case of an empty list of schemas is provided
  # jsonencode is used to quote each schema name additionally to prevent issue with sql syntax
  schemas = length(var.schemas) == 0 ? "ALL" : join(",", [
    for schema in var.schemas : jsonencode(schema)
  ])
