...
BEGIN
  ...
  IF affected_schemas = 'ALL' THEN
    ...
    LOOP
      EXECUTE FORMAT('GRANT %s ON SCHEMA "%s" TO "${group_role}"', granted_privileges, each_schema);
      RAISE NOTICE '% privileges were granted on schema %.', granted_privileges, each_schema;
    END LOOP;
  ELSE
    EXECUTE FORMAT('GRANT %s ON SCHEMA %s TO "${group_role}"', granted_privileges, affected_schemas);
